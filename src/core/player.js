...
DM.provide('', {
  player: function(element, options) {
    ...
    if (!options || typeof options !== 'object') throw new Error("Missing 'options' parameter for DM.player()");
  ...
  destroy: function(id) {
    if (!id) {
      // destroy all players of the page
...
DM.provide('Player', {
  ...
  create: function(element, options) {
    ...
    DM.Player._PROTOCOL =
      window.location && /^https?:$/.test(window.location.protocol) ? window.location.protocol : 'http:';
  ...
  init: function(video, params, playlist, events, element) {
    ...
    if (location.origin) params.origin = location.origin;
    else params.origin = '*';
  ...
  _recvEvent: function(event) {
    switch (event.event) {
      case 'apiready':
        ...
        else this.apiReady = true;
        this._environmentInfo = event.info || null;
        break;
      case 'start':
        this.ended = false;
        break;
      ...
      case 'end':
        this.ended = true;
        break; // no break, also set paused
      case 'ad_pause':
      case 'ad_end':
      case 'video_end':
      case 'pause':
        ...
        break;
      case 'error':
        this.error = { code: event.code, title: event.title, message: event.message };
        break;
      case 'rebuffer':
      ...
      case 'subtitlechange':
        ...
        break;
      case 'videochange':
        this.video = { videoId: event.videoId, title: event.title };
        break;
      case 'ad_companions':
